...
class DeactivateProductUseCase @Inject constructor(
    ...
    suspend operator fun invoke(productId: String, user: User): Result<Unit> {
        if (user.role != Role.ADMIN) {
            return Result.Error(AppError.Unauthorized("Hanya Admin yang dapat menonaktifkan produk."))
        }
        ...
        return try {
            productRepository.setProductActiveStatus(productId, false)
            Result.Success(Unit)
        } catch (e: Exception) {
            Result.Error(AppError.Database("Gagal menonaktifkan produk: ${e.message}", e))
        }
    }
}
